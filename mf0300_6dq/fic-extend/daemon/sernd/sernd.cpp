...
int cmd_parser(char *szCmd, int nSocket) {
	...
	if (tok != NULL) {
		...
		for (i=0; i<ncmd; i++) {
			...
			cmdlen = strlen(g_cmdMap[i].szCmd) > strlen(tok) ?
			          strlen(g_cmdMap[i].szCmd) : strlen(tok);
			/* parser command */
...
void * actionThread(void *pParam) {
	...
	char sz[1024]={0};
	if (0 < recv(nSocket, sz, sizeof(sz)-1, 0)) {
			DEBUG_MSG("recv: %s\n", sz);
...
int main () {
	...
	if (0 == pthread_create(&mem_threadid, NULL, getPhyMemThread, NULL)) {
		DEBUG_MSG("memory thread created\n");
	}	
	
	int nSocket = android_get_control_socket("serialnumber");
	if (-1 == listen(nSocket, MAX_CONNECTION)) {
	...
	if (CheckMotherBoard()==0){
		...
		if (0 == pthread_create(&check_threadid, NULL, CheckTimeThread, NULL)) {
	...
	while (1) {		
		...
		if (0 == pthread_create(&threadid, NULL, actionThread, reinterpret_cast<void*>(nClientSocket))) {
